...
class unsplash:
	...
	def input_search_terms(self):
		...
		self.search_terms = quote(raw_input());
	...
	def run(self):
		...
		for i in range(1, 10000000):
			...
			while not self.threads.empty():
				...
				time.sleep(0.3);
			if self.debug_mode:
				print "all threads opened, queue.size = %d" % (self.threads.qsize());
...
# test = unsplash(_debug_mode = True, _show_pct_bar = False, _hashes_per_page = 5, _download_time_out = 10, _max_items_cnt = 6);
test = unsplash();
test.run();
