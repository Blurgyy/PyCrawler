...
class unsplash:
	...
	def get_dl_links_masked(self):
		try:
			...
			self.all_hash = list(set(re.findall(self.pattern, self.src_code.decode('utf-8'))));
	...
	def download(self, link, fpath, fname):
		# self.req = urllib.request.Request(link, headers = headers);
		# self.resp = urllib.request.urlopen(self.req);
		...
		urllib.request.urlretrieve(link, dl_location, completion);
